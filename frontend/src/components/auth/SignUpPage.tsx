import React, { useState, useEffect } from 'react';
import { Link, useNavigate, useLocation } from 'react-router-dom';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { z } from 'zod';
import {
  Eye, EyeOff, Mail, Lock, User, Loader2,
  AlertCircle, CheckCircle2, ArrowRight
} from 'lucide-react';
import AuthLayout from './AuthLayout';
import { useAuth } from '../../hooks/useAuth';
import { authAPI } from '../../services/authService';

const PASSWORD_RE = {
  upper: /[A-Z]/,
  lower: /[a-z]/,
  digit: /\d/,
  special: /[!@#$%^&*()\-_=+[\]{}|;:'",.<>?/\\`~]/,
};

const schema = z
  .object({
    full_name: z.string().min(2, 'Name must be at least 2 characters'),
    email: z.string().email('Please enter a valid email'),
    password: z
      .string()
      .min(8, 'Password must be at least 8 characters')
      .refine((v) => PASSWORD_RE.upper.test(v), 'Must contain an uppercase letter')
      .refine((v) => PASSWORD_RE.lower.test(v), 'Must contain a lowercase letter')
      .refine((v) => PASSWORD_RE.digit.test(v), 'Must contain a digit')
      .refine((v) => PASSWORD_RE.special.test(v), 'Must contain a special character'),
    confirm_password: z.string(),
    terms: z.literal(true, { errorMap: () => ({ message: 'You must accept the terms' }) }),
  })
  .refine((d) => d.password === d.confirm_password, {
    message: 'Passwords do not match',
    path: ['confirm_password'],
  });

type FormData = z.infer<typeof schema>;

function getStrength(password: string): number {
  if (!password) return 0;
  let score = 0;
  if (password.length >= 8) score++;
  if (PASSWORD_RE.upper.test(password)) score++;
  if (PASSWORD_RE.lower.test(password)) score++;
  if (PASSWORD_RE.digit.test(password)) score++;
  if (PASSWORD_RE.special.test(password)) score++;
  return score;
}

const STRENGTH_LABELS = ['', 'Very Weak', 'Weak', 'Fair', 'Strong', 'Very Strong'];
const STRENGTH_COLORS = ['', '#ef4444', '#f97316', '#eab308', '#22c55e', '#10a37f'];

/* ── OAuth Provider Icons ─────────────────────────────────────────────────── */
function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
      <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
      <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
      <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
      <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
    </svg>
  );
}

function GitHubIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" />
    </svg>
  );
}

function MicrosoftIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
      <rect x="1" y="1" width="10.5" height="10.5" fill="#f25022" rx="0.8"/>
      <rect x="12.5" y="1" width="10.5" height="10.5" fill="#7fba00" rx="0.8"/>
      <rect x="1" y="12.5" width="10.5" height="10.5" fill="#00a4ef" rx="0.8"/>
      <rect x="12.5" y="12.5" width="10.5" height="10.5" fill="#ffb900" rx="0.8"/>
    </svg>
  );
}

export default function SignUpPage() {
  const { register: registerUser, isLoading, error, clearError } = useAuth();
  const location = useLocation();
  const [showPassword, setShowPassword] = useState(false);
  const [showConfirm, setShowConfirm] = useState(false);
  const [success, setSuccess] = useState(false);

  // Extract pre-filled email from location state if we navigated here from the login page's smart prompt
  const prefillEmail = (location.state as { prefillEmail?: string })?.prefillEmail || '';

  const { register, handleSubmit, formState: { errors, isValid }, watch, setValue } = useForm<FormData>({
    resolver: zodResolver(schema),
    mode: 'onChange',
    defaultValues: {
      email: prefillEmail,
    }
  });

  // Watch fields for dynamic styling
  const emailValue = watch('email');
  const passwordValue = watch('password', '');
  const nameValue = watch('full_name');

  useEffect(() => {
    return () => clearError();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const onSubmit = async (data: FormData) => {
    clearError();
    try {
      await registerUser(data.full_name, data.email, data.password, data.confirm_password);
      setSuccess(true);
    } catch {
      // Error shown from store
    }
  };

  const handleOAuth = (provider: 'google' | 'github' | 'microsoft') => {
    window.location.href = authAPI.getOAuthUrl(provider);
  };

  const strength = getStrength(passwordValue);

  // ── Success State ──────────────────────────────────────────────────────────
  if (success) {
    return (
      <AuthLayout title="Check your inbox" subtitle="We've sent you a verification email">
        <div className="auth-success-state" style={{ padding: '20px 0', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '24px' }}>
          <div className="auth-success-icon" style={{ 
            width: '80px', height: '80px', borderRadius: '50%', 
            background: 'rgba(16,163,127,0.1)', border: '1px solid rgba(16,163,127,0.2)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            color: '#10a37f'
          }}>
            <CheckCircle2 size={40} strokeWidth={1.5} />
          </div>
          <p className="auth-success-text" style={{ textAlign: 'center', color: 'rgba(156,163,175,0.9)', fontSize: '15px', lineHeight: '1.6' }}>
            A verification link has been sent to your email. Click it to activate your account
            and start filing FIRs with NESYA.
          </p>
          <Link to="/login" className="lp-submit-btn lp-submit-btn--active" style={{ textDecoration: 'none' }}>
            Go to Sign In
          </Link>
        </div>
      </AuthLayout>
    );
  }

  return (
    <AuthLayout title="Create an account" subtitle="Join NESYA and start filing AI-powered FIRs">
      {error && (
        <div className="lp-alert lp-alert--error" role="alert" aria-live="assertive">
          <AlertCircle size={15} className="lp-alert-icon" />
          <span>{error}</span>
        </div>
      )}

      {/* ── OAuth Row ──────────────────────────────────────── */}
      <div className="lp-oauth-row">
        <button
          type="button"
          className="lp-oauth-btn"
          onClick={() => handleOAuth('google')}
          id="signup-google"
          aria-label="Sign up with Google"
        >
          <GoogleIcon />
          <span>Google</span>
        </button>
        <button
          type="button"
          className="lp-oauth-btn"
          onClick={() => handleOAuth('github')}
          id="signup-github"
          aria-label="Sign up with GitHub"
        >
          <GitHubIcon />
          <span>GitHub</span>
        </button>
        <button
          type="button"
          className="lp-oauth-btn"
          onClick={() => handleOAuth('microsoft')}
          id="signup-microsoft"
          aria-label="Sign up with Microsoft"
        >
          <MicrosoftIcon />
          <span>Microsoft</span>
        </button>
      </div>

      {/* ── Divider ────────────────────────────────────────── */}
      <div className="lp-divider" aria-hidden="true">
        <span className="lp-divider-line" />
        <span className="lp-divider-text">or sign up with email</span>
        <span className="lp-divider-line" />
      </div>

      {/* ── Form ───────────────────────────────────────────── */}
      <form onSubmit={handleSubmit(onSubmit)} className="lp-form" noValidate>
        {/* Full Name */}
        <div className="lp-field">
          <label className="lp-label" htmlFor="signup-name">Full name</label>
          <div className={`lp-input-wrap ${errors.full_name ? 'lp-input-wrap--error' : nameValue ? 'lp-input-wrap--filled' : ''}`}>
            <User size={16} className="lp-input-icon" />
            <input
              id="signup-name"
              type="text"
              className="lp-input"
              placeholder="Ravi Kumar"
              autoComplete="name"
              spellCheck={false}
              {...register('full_name')}
            />
          </div>
          {errors.full_name && (
            <span className="lp-field-error" role="alert">
              <AlertCircle size={12} />
              {errors.full_name.message}
            </span>
          )}
        </div>

        {/* Email */}
        <div className="lp-field">
          <label className="lp-label" htmlFor="signup-email">Email address</label>
          <div className={`lp-input-wrap ${errors.email ? 'lp-input-wrap--error' : emailValue ? 'lp-input-wrap--filled' : ''}`}>
            <Mail size={16} className="lp-input-icon" />
            <input
              id="signup-email"
              type="email"
              className="lp-input"
              placeholder="you@example.com"
              autoComplete="email"
              spellCheck={false}
              {...register('email')}
            />
          </div>
          {errors.email && (
            <span className="lp-field-error" role="alert">
              <AlertCircle size={12} />
              {errors.email.message}
            </span>
          )}
        </div>

        {/* Password */}
        <div className="lp-field">
          <label className="lp-label" htmlFor="signup-password">Password</label>
          <div className={`lp-input-wrap ${errors.password ? 'lp-input-wrap--error' : ''}`}>
            <Lock size={16} className="lp-input-icon" />
            <input
              id="signup-password"
              type={showPassword ? 'text' : 'password'}
              className="lp-input"
              placeholder="Create a strong password"
              autoComplete="new-password"
              {...register('password')}
            />
            <button
              type="button"
              className="lp-toggle-btn"
              onClick={() => setShowPassword(v => !v)}
              aria-label={showPassword ? 'Hide password' : 'Show password'}
              tabIndex={0}
            >
              {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
            </button>
          </div>
          
          {/* Password strength meter */}
          {passwordValue && (
            <div className="auth-strength" style={{ marginTop: '8px' }}>
              <div className="auth-strength-bars" style={{ display: 'flex', gap: '4px', flex: 1 }}>
                {[1, 2, 3, 4, 5].map((i) => (
                  <div
                    key={i}
                    style={{
                      flex: 1,
                      height: '4px',
                      borderRadius: '2px',
                      background: i <= strength ? STRENGTH_COLORS[strength] : 'rgba(255,255,255,0.1)',
                      transition: 'background 0.3s ease'
                    }}
                  />
                ))}
              </div>
              <span style={{ fontSize: '11.5px', fontWeight: 500, minWidth: '70px', textAlign: 'right', color: STRENGTH_COLORS[strength] || 'rgba(156,163,175,0.8)' }}>
                {STRENGTH_LABELS[strength] || 'Too short'}
              </span>
            </div>
          )}

          {errors.password && (
            <span className="lp-field-error" role="alert">
              <AlertCircle size={12} />
              {errors.password.message}
            </span>
          )}
        </div>

        {/* Confirm Password */}
        <div className="lp-field">
          <label className="lp-label" htmlFor="signup-confirm">Confirm password</label>
          <div className={`lp-input-wrap ${errors.confirm_password ? 'lp-input-wrap--error' : ''}`}>
            <Lock size={16} className="lp-input-icon" />
            <input
              id="signup-confirm"
              type={showConfirm ? 'text' : 'password'}
              className="lp-input"
              placeholder="Repeat your password"
              autoComplete="new-password"
              {...register('confirm_password')}
            />
            <button
              type="button"
              className="lp-toggle-btn"
              onClick={() => setShowConfirm(v => !v)}
              aria-label={showConfirm ? 'Hide password' : 'Show password'}
              tabIndex={0}
            >
              {showConfirm ? <EyeOff size={16} /> : <Eye size={16} />}
            </button>
          </div>
          {errors.confirm_password && (
            <span className="lp-field-error" role="alert">
              <AlertCircle size={12} />
              {errors.confirm_password.message}
            </span>
          )}
        </div>

        {/* Terms */}
        <div className="lp-field" style={{ marginTop: '4px' }}>
          <label className="lp-checkbox-label">
            <input type="checkbox" className="lp-checkbox" {...register('terms')} />
            <span className="lp-checkbox-custom" aria-hidden="true" />
            <span className="lp-checkbox-text">
              I agree to the{' '}
              <a href="#" style={{ color: '#10a37f', textDecoration: 'none' }}>Terms of Service</a>
              {' '}and{' '}
              <a href="#" style={{ color: '#10a37f', textDecoration: 'none' }}>Privacy Policy</a>
            </span>
          </label>
          {errors.terms && (
            <span className="lp-field-error" role="alert">
              <AlertCircle size={12} />
              {errors.terms.message}
            </span>
          )}
        </div>

        {/* Submit */}
        <button
          type="submit"
          className={`lp-submit-btn ${isValid && !isLoading ? 'lp-submit-btn--active' : ''}`}
          disabled={isLoading || success}
          id="signup-submit"
        >
          {isLoading ? (
            <>
              <Loader2 size={16} className="lp-spin" />
              <span>Creating account…</span>
            </>
          ) : (
            <>
              <span>Create account</span>
              <ArrowRight size={16} className="lp-btn-arrow" />
            </>
          )}
        </button>
      </form>

      {/* Sign in link */}
      <p className="lp-footer">
        Already have an account?{' '}
        <Link to="/login" className="lp-footer-link">
          Sign in
        </Link>
      </p>
    </AuthLayout>
  );
}
